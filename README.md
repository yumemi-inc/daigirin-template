...
`yarn build` / `yarn build:press` 実行時は **不足しているファイルのみ** 生成します。すでに存在するファイルは上書きしないため、手動修正を保持できます。

全ファイルを再生成して上書きしたい場合は、次のコマンドを実行します。
...
- `before`（デフォルト）: 記事一覧の前に表示されます。省略時も `before` 扱いです。
- `after`: 記事一覧の後ろに表示されます。

```yaml
- title: はじめに
  file: pages/preface.html
- title: おわりに
  file: pages/afterword.html
  position: after
```
